...
export class SidebarPrefetcher {
  ...
  getSidebarDataInstantly(userId: string): SidebarPrefetchData | null {
    ...
    try {
      ...
      console.error(
        "[SidebarPrefetch] Failed to get instant sidebar data:",
        error,
      );
    }
  ...
  private computePageAccess(permissions: any[]) {
    return {
      dashboard: true,
      products: this.hasPermission(
        permissions,
        RESOURCES.PRODUCTS,
        PERMISSIONS.VIEW,
      ),
      orders: this.hasPermission(
        permissions,
        RESOURCES.ORDERS,
        PERMISSIONS.VIEW,
      ),
      clients: this.hasPermission(
        permissions,
        RESOURCES.CLIENTS,
        PERMISSIONS.VIEW,
      ),
      companies: this.hasPermission(
        permissions,
        RESOURCES.COMPANIES,
        PERMISSIONS.VIEW,
      ),
      inventory: this.hasPermission(
        permissions,
        RESOURCES.INVENTORY,
        PERMISSIONS.VIEW,
      ),
  ...
  private computeActions(permissions: any[]) {
    return {
      products: {
        view: this.hasPermission(
          permissions,
          RESOURCES.PRODUCTS,
          PERMISSIONS.VIEW,
        ),
        create: this.hasPermission(
          permissions,
          RESOURCES.PRODUCTS,
          PERMISSIONS.CREATE,
        ),
        edit: this.hasPermission(
          permissions,
          RESOURCES.PRODUCTS,
          PERMISSIONS.EDIT,
        ),
